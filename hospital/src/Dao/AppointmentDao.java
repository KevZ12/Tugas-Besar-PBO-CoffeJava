...
public class AppointmentDao implements IDaoAppointment {
    ...
    //ngecek jika dokter sudah ada jadwal di tanggal segitu
    @Override
    public boolean checkDate(LocalDate date, int docId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM appoiments WHERE Date = ? AND DoctorId = ?";
    try (Connection connection = DataBaseConnection.getConnection(); 
         PreparedStatement statement = connection.prepareStatement(sql)) {
        ...
        e.printStackTrace();
        throw new SQLException("Error while checking date: " + e.getMessage());
    }
    return false;
    }

    
    //ngecek jika room pada tanggal hari itu sudah dipakai
    @Override
    public boolean checkRoom(LocalDate date, int roomId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM appoiments WHERE Date = ? AND RoomId = ?";
    try (Connection connection = DataBaseConnection.getConnection(); 
         PreparedStatement statement = connection.prepareStatement(sql)) {
        statement.setDate(1, java.sql.Date.valueOf(date));
        ...
        e.printStackTrace();
        throw new SQLException("Error while checking room: " + e.getMessage());
    }
    return false;
    }
}
