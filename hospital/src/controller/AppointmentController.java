...
public class AppointmentController {
    ...
    public void add(LocalTime start, LocalTime end, int status, int isDone, int cap, int room, int docId, LocalDate date) throws SQLException {
        ...
        if (!start.isBefore(end)) {
            JOptionPane.showMessageDialog(null, "Invalid Input for start session");
            return;
        }
        ...
        if (dao.checkDate(date, docId)){
            JOptionPane.showMessageDialog(null, "Dokter mempunyai janji temu yang dijadwalkan pada tanggal tersebut");
            return;
        }
        
        if (dao.checkRoom(date, room)){
            JOptionPane.showMessageDialog(null, "Room already booked, please choose another room");
            return;
        }
